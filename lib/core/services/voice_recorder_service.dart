...
class VoiceRecorderService {
  ...
  static const int hopSize = 2205; // 0.05 секунды шаг
  ...
  for (int i = 0; i < notes.length; i++) {
    final note = notes[i];
    final newStartTick = note.startTick - minStartTick;
    
    // Для первой ноты корректируем длительность, если она выходит за границы
    int newDurationTicks = note.durationTicks;
    if (i == 0) {
      // Проверяем, не "вылезает" ли первая нота за пределы
      // Максимальная длительность первой ноты не должна превышать startTick следующей ноты (если есть)
      if (notes.length > 1) {
        final nextNoteStart = notes[1].startTick;
        final maxDuration = nextNoteStart - note.startTick;
        if (newDurationTicks > maxDuration) {
          newDurationTicks = maxDuration;
          debugPrint('✂️ Обрезана длительность первой ноты с ${note.durationTicks} до $newDurationTicks');
        }
      }
    }
